...
class PrefUtil {
    companion object{
        ...
        fun getTimerState(context: Context): TimerActivity.Statetimer{
            val preferences = PreferenceManager.getDefaultSharedPreferences(context)
            val ordinal = preferences.getInt(TIMER_STATE_ID, 0)
            return TimerActivity.Statetimer.values()[ordinal]
        }

        fun setTimerState(state: TimerActivity.Statetimer, context: Context){
            val editor = PreferenceManager.getDefaultSharedPreferences(context).edit()
