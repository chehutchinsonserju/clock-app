...
class PrefUtil {
    companion object{
        ...
        fun getTimerState(context: Context): MainActivity.Statetimer{
            val preferences = PreferenceManager.getDefaultSharedPreferences(context)
            val ordinal = preferences.getInt(TIMER_STATE_ID, 0)
            return MainActivity.Statetimer.values()[ordinal]
        }

        fun setTimerState(state: MainActivity.Statetimer, context: Context){
            val editor = PreferenceManager.getDefaultSharedPreferences(context).edit()
